...
#   <REDIS_*_HOST>                 : terraform output redis_*_endpoint
#
# 사용:
...
REDIS_RANKING_HOST="${REDIS_RANKING_HOST:-}"
REDIS_PRICE_CACHE_HOST="${REDIS_PRICE_CACHE_HOST:-}"
REDIS_MARKET_PUBSUB_HOST="${REDIS_MARKET_PUBSUB_HOST:-}"
if command -v terraform >/dev/null 2>&1 && [ -d "$TF_DIR" ]; then
  ...
  [ -z "$REDIS_RANKING_HOST" ] && REDIS_RANKING_HOST="$(terraform -chdir="$TF_DIR" output -raw redis_ranking_endpoint 2>/dev/null || true)"
  [ -z "$REDIS_PRICE_CACHE_HOST" ] && REDIS_PRICE_CACHE_HOST="$(terraform -chdir="$TF_DIR" output -raw redis_price_cache_endpoint 2>/dev/null || true)"
  [ -z "$REDIS_MARKET_PUBSUB_HOST" ] && REDIS_MARKET_PUBSUB_HOST="$(terraform -chdir="$TF_DIR" output -raw redis_market_pubsub_endpoint 2>/dev/null || true)"
fi
...
echo "REDIS_RANKING_HOST=${REDIS_RANKING_HOST:-<미해결: terraform output 필요>}"
echo "REDIS_PRICE_CACHE_HOST=${REDIS_PRICE_CACHE_HOST:-<미해결: terraform output 필요>}"
echo "REDIS_MARKET_PUBSUB_HOST=${REDIS_MARKET_PUBSUB_HOST:-<미해결: terraform output 필요>}"
echo "---"
...
while IFS= read -r f; do
  ...
  [ -n "$REDIS_RANKING_HOST" ] && sed_i "s|<REDIS_RANKING_HOST>|${REDIS_RANKING_HOST}|g" "$f"
  [ -n "$REDIS_PRICE_CACHE_HOST" ] && sed_i "s|<REDIS_PRICE_CACHE_HOST>|${REDIS_PRICE_CACHE_HOST}|g" "$f"
  [ -n "$REDIS_MARKET_PUBSUB_HOST" ] && sed_i "s|<REDIS_MARKET_PUBSUB_HOST>|${REDIS_MARKET_PUBSUB_HOST}|g" "$f"
  echo "rendered: ${f#$ROOT/}"
  changed=1
done < <(grep -rlE '<ACCOUNT_ID>|<ECR>|<MSK_IAM_BOOTSTRAP>|<DEBEZIUM_ROLE_ARN>|<REDIS_RANKING_HOST>|<REDIS_PRICE_CACHE_HOST>|<REDIS_MARKET_PUBSUB_HOST>' "$TARGET_DIR" 2>/dev/null || true)

[ "$changed" -eq 0 ] && echo "치환할 placeholder 없음(이미 렌더됨)."

# 남은 미해결 placeholder 경고
remaining="$(grep -rEo '<MSK_IAM_BOOTSTRAP>|<DEBEZIUM_ROLE_ARN>|<REDIS_RANKING_HOST>|<REDIS_PRICE_CACHE_HOST>|<REDIS_MARKET_PUBSUB_HOST>' "$TARGET_DIR" 2>/dev/null | sort -u || true)"
if [ -n "$remaining" ]; then
  echo ""
  echo "⚠️ 미해결(아직 placeholder): $remaining"
  echo "   → dev apply 후 'terraform output'으로 값이 나오면 다시 실행하거나"
  echo "     MSK_BOOTSTRAP=... DEBEZIUM_ROLE_ARN=... REDIS_RANKING_HOST=... scripts/render-placeholders.sh $ENV"
fi
